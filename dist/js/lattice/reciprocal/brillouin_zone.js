...
exports.computeBrillouinZone = computeBrillouinZone;
/**
...
function solveLinearSystem(matrix, rightHandSide) {
    ...
    const determinantWithColumnReplaced = (columnIndex) => {
 ...
 */
function computeBrillouinZone(reciprocalVectors) {
    if (reciprocalVectors.length !== 3) {
        return null;
    }
    const [firstVector, secondVector, thirdVector] = reciprocalVectors;
    const isDegenerate = reciprocalVectors.some((vector) => vector.length !== 3 || vector.some((component) => !Number.isFinite(component)));
    if (isDegenerate) {
        return null;
    }
    const latticePoints = [];
    ...
    const isInsideZone = (point) => planes.every((plane) => dotProduct(point, plane.normal) <= plane.offset + HALF_SPACE_TOLERANCE);
    const vertices = [];
    for (let first = 0; first < planes.length; first += 1) {
        for (let second = first + 1; second < planes.length; second += 1) {
            for (let third = second + 1; third < planes.length; third += 1) {
                const point = solveLinearSystem([planes[first].normal, planes[second].normal, planes[third].normal], [planes[first].offset, planes[second].offset, planes[third].offset]);
                const isZoneVertex = point !== null && isInsideZone(point);
                const isDuplicate = isZoneVertex &&
                    vertices.some((existing) => vectorLength(subtract(existing, point)) <
    ...
    planes.forEach((plane) => {
        const verticesOnPlane = vertices.filter((vertex) => Math.abs(dotProduct(vertex, plane.normal) - plane.offset) < ON_PLANE_TOLERANCE);
        ...
        const centroid = scale(verticesOnPlane.reduce((sum, vertex) => [sum[0] + vertex[0], sum[1] + vertex[1], sum[2] + vertex[2]], [0, 0, 0]), 1 / verticesOnPlane.length);
        const inPlaneAxis = normalize(subtract(verticesOnPlane[0], centroid));
        const inPlaneBitangent = crossProduct(normal, inPlaneAxis);
        const angleAboutNormal = (vertex) => {
            const offsetFromCentroid = subtract(vertex, centroid);
            return Math.atan2(dotProduct(offsetFromCentroid, inPlaneBitangent), dotProduct(offsetFromCentroid, inPlaneAxis));
        };
        const ordered = [...verticesOnPlane].sort((left, right) => angleAboutNormal(left) - angleAboutNormal(right));
