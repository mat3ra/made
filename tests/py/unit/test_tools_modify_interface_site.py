...
def test_atoms_within_a_radius_of_a_coordinate_nearest_first():
    ...
    assert get_atom_indices_within_radius_of_coordinate(mos2, [0.6667, 0.3333, 0.45], 1.0, "S") == [1]
    assert get_atom_indices_within_radius_of_coordinate(mos2, [0.6667, 0.3333, 0.45], 3.0, "S") == [1, 2]
    assert get_atom_indices_within_radius_of_coordinate(mos2, [0.6667, 0.3333, 0.45], 0.1, "S") == []


def test_closest_site_within_radius_by_element():
    mos2 = Material.create(MOS2)
    assert get_closest_site_id_within_radius(mos2, [0.25, 0.75, 0.5], 1.0, "Mo") == 0
    assert get_closest_site_id_within_radius(mos2, [0.6, 0.3, 0.58], 1.0, "S") == 2
    with pytest.raises(ValueError, match=r"No Mo within 0.5 A .* nearest Mo is 1\.\d\d A away"):
        get_closest_site_id_within_radius(mos2, [0.0, 0.0, 0.5], 0.5, "Mo")
...
def test_film_over_two_neighbours_is_a_bridge():
    ...
    placed = interface_displace_film_to_site(supercell, carbon, top_nickel(supercell)[:2])
...
def test_film_over_three_neighbours_is_a_hollow():
    supercell = create_supercell(INTERFACE, scaling_factor=[2, 2, 1])
    carbon = first_carbon(supercell)
    placed = interface_displace_film_to_site(supercell, carbon, top_nickel(supercell)[:3])
