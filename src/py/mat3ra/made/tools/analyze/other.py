...
def get_atom_indices_within_radius_pbc(
    ...
    points at an atom: "the Mo near (0.25, 0.25, 0.5)". Otherwise it is centred on `atom_index`.
    ...
    for site in sites_within_radius:
        if chemical_element is not None and site.species_string != chemical_element:
            continue
        if site.index not in selected_indices:
            selected_indices.append(site.index)
    return selected_indices
